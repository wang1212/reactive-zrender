 ...
 * Copy from ECharts
 *
 * @see https://github.com/apache/echarts/blob/5.3.3/src/component/helper/RoamController.ts
 */
import * as zrender from 'zrender';
import Eventful from 'zrender/lib/core/Eventful';
import * as eventTool from 'zrender/lib/core/event';
import { type ZRenderType } from 'zrender/lib/zrender';
import { Bind3, isString, bind, defaults, clone } from 'zrender/lib/core/util';
import type Group from 'zrender/lib/graphic/Group';
import * as interactionMutex from './interactionMutex';

type ZRElementEvent = zrender.ElementEvent;
...
class RoamController extends Eventful<{
  ...
  private _mousewheelHandler(e: ZRElementEvent) {
    ...
    if (shouldMove) {
      ...
      // wheelDelta of mouse wheel is bigger than touch pad.
      const scrollDelta =
        (wheelDelta > 0 ? 1 : -1) * (absDelta > 3 ? 0.4 : absDelta > 1 ? 0.15 : 0.05);
